...
$$
  L = \mathbb{E}_{q} \left[ -\text{log }p(x_T) - \sum_{t \geq 1}^{T} \text{log } \frac{p_{\theta}(x_{t-1}|x_t)}{q(x_t|x_{t-1})} \right]
$$
...
The terms $$ q(x_t) $$ and $$ q(x_{t-1}) $$ are intractable to compute, as they would require marginalising over all data points. 
However, due to the markov propety of the forward process we have:
...
First, lets continue our work on the ELBO. Separating the contribution from the
...
These cancellations lead to the following bound:

$$
  \mathbb{E}_{q} \left[ 
    \underbrace{-\text{log }p(x_T) + \text{log }q(x_T|x_0)}_{L_T} - 
    \underbrace{\text{log } p_{\theta}(x_0|x_1)}_{L_0} - 
    \underbrace{\sum_{t \gt 1}^{T} \text{log } \frac{p_{\theta}(x_{t-1}|x_t)}{q(x_{t-1}|x_t,x_0)}}_{L_{1:T-1}}
  \right]
$$

$$ L_T $$ has no parameters, and simply measures the effectiveness of the forward process in producing a standard gaussian. 
$$ L_0 $$ is the reconstruction term, and the summand represents the divergences between each generative layer and the 
corresponding forward process posteriors.
